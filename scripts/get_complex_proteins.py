...
# input/output files
infile_complex='resources/complex/MusMusculus.tsv'
infile_chiptf='resources/experimentList_mm10_TFs.tab'
outfile='results/TF_Complex.tsv'

# read tables
complex_table = pd.read_csv(infile_complex,sep='\t')
chip_tf = np.unique( np.array(pd.read_csv(infile_chiptf,sep='\t',header=None,usecols=[3])[3].values ))
...
pd.DataFrame(interaction_matrix,columns=chip_tf,index=chip_tf).to_csv(outfile,sep="\t")